...
export default function TagCreateModal({ onClose, onCreated }) {
  ...
  return createPortal(
    ...
    >
      <div
        className="relative w-full max-w-md card-surface bg-white dark:bg-ink-900 animate-slide-up overflow-hidden"
      ...
      >
        <div className="border-b border-paper-300/60 dark:border-ink-700/60 px-5 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="text-[13px] font-medium text-zinc-900 dark:text-zinc-100">Tag mới</div>
          </div>
          <button
            onClick={onClose}
              ...
              {COLORS.map((c) => (
                <button
                  ...
                  className={`w-7 h-7 rounded-full ${c.dot} flex items-center justify-center transition ${
                    color === c.name
                      ? 'ring-2 ring-offset-2 ring-offset-white dark:ring-offset-ink-900 ring-zinc-400 scale-110'
        ...
        <div className="border-t border-paper-300/60 dark:border-ink-700/60 px-5 py-3 flex items-center justify-between bg-paper-50/60 dark:bg-ink-900/60">
          <div className="text-[10.5px] text-zinc-500 flex items-center gap-1.5">
            ...
            <span>Xem trước:</span>
            <span className="text-zinc-700 dark:text-zinc-300 font-medium">#{slug || 'tag-mới'}</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
